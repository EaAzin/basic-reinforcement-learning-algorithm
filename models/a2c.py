...
class Actor(nn.Module):
    def __init__(self,state_dim,hidden_dim,action_dim):
        ...
        self.fc1 = nn.Linear(16 * 22 * 64, hidden_dim)
...
def train(args=get_args()):
    ...
    env = wrappers.RecordVideo(env=env,video_folder='/tmp/exp-4',episode_trigger= lambda i_episcode: i_episcode % 50 == 0 )
    ...
    args.state_dim = env.observation_space.shape[2]
    ...
    return_list = []
    for i in range(10):
        log_dir = f"ReinforcementLearning/A2C/runs2/gamma_{args.gamma} alr_{args.actor_lr} clr_{args.critic_lr}/_iter_{i}"
        ...
        with tqdm(total=int(args.num_episodes/10),desc="Iteration %d" % i ) as pbar:
            for i_episode in range(int(args.num_episodes/10)):
                ...
                episode_return = 0
                state = env.reset()
                #states = [env.reset() for _ in range(args.batch_size)]
                state = np.transpose(state[0],(2,1,0))
                #states = [state[0] for state in states]
                done = False
                #done = torch.zeros((args.batch_size,),dtype=torch.bool)
                transition_dict = {
                        "states":[],
                        "actions":[],
                        "rewards":[],
                ...
                #transition_dict[key] = torch.stack(transition_dict[key],dim=1)
                writer.add_scalar("return",episode_return,i_episode + i * int(args.num_episodes/10))
